...
if which "$project_exec" >/dev/null; then
	...
	if [[ "$resposta" =~ ^[sS]$ ]];then
		...
		sleep 2
		git clone https://github.com/LeonardoCardosoDev/Individual_Analyx.git #executa o clone onde esta o jar
		cd Individual_Analyx #acessa o diretorio java
		...
		sudo mv analyx-source-gui-1.0-SNAPSHOT-jar-with-dependencies.jar ./Assistente_Analyx_Docker
		sleep 2
		echo -e "${CIANO}${NEGRITO}[Assistente Analyx]${RESET}${VERDE}INSTALAÇÃO REALIZADA COM SUCESSO :D${RESET_FONTE}${RESET}"
	else
		echo -e "${CIANO}${NEGRITO}[Assistente Analyx]${RESET}${RESET_FONTE}${AMARELO}${ITALICO}${NEGRITO}Tudo bem! , até a próxima ;)${RESET_FONTE}${RESET}"
